...
def manage_trade_exits(client):
  ...
  for position in open_positions_dict:
    ...
    markets = client.public.get_markets().data # Whenever you do an API call, do a check to see whether it returned data, if so do x, if not do y
  ...
  ###
    ...
    # Close positions if triggered / IF DEV!, not is_close. 
    if is_close:
