...
def calculate_half_life(spread):
  ...
  spread_lag = df_spread.spread.shift(1) # -1 future
...
def store_cointegration_results(df_market_prices):
  ...
  for index, base_market in enumerate(markets[:-1]):
    series_1 = df_market_prices[base_market].dropna().values.astype(float).tolist()
  
    # get quote pair
    for quote_market in markets[index + 1:]:
      series_2 = df_market_prices[quote_market].dropna().values.astype(float).tolist()
  ...
  del df_critera_met # to clear space in memory
